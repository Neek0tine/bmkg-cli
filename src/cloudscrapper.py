...
class cloudscrapper:
    ...
    def get_weather(self):
        ...
        async def _extractor(request):
            ...
            today = pd.DataFrame({'city': city_name, 'date': date_list[0], 'time': times_list[:-34], 'status': conditions_list[:-34],
                                  'temp': temp_list[:-34], 'humid': humid_list[:-34], 'wind': wind_list[:-34]})
            day2 = pd.DataFrame({'city': city_name, 'date': date_list[-6], 'time': times_list[-34:-26], 'status': conditions_list[-34:-26],
                                 'temp': temp_list[-34:-26], 'humid': humid_list[-34:-26], 'wind': wind_list[-34:-26]})
            day3 = pd.DataFrame({'city': city_name, 'date': date_list[-5], 'time': times_list[-26:-18], 'status': conditions_list[-26:-18],
                                 'temp': temp_list[-26:-18], 'humid': humid_list[-26:-18], 'wind': wind_list[-26:-18]})
            day4 = pd.DataFrame({'city': city_name, 'date': date_list[-4], 'time': times_list[-17:-12], 'status': conditions_list[-17:-12],
                                 'temp': temp_list[-17:-12], 'humid': humid_list[-17:-12], 'wind': wind_list[-17:-12]})
            day5 = pd.DataFrame({'city': city_name, 'date': date_list[-3], 'time': times_list[-12:-8], 'status': conditions_list[-12:-8],
                                 'temp': temp_list[-12:-8], 'humid': humid_list[-12:-8], 'wind': wind_list[-12:-8]})
            day6 = pd.DataFrame({'city': city_name, 'date': date_list[-2], 'time': times_list[-8:-4], 'status': conditions_list[-8:-4],
                                 'temp': temp_list[-8:-4], 'humid': humid_list[-8:-4], 'wind': wind_list[-8:-4]})
            day7 = pd.DataFrame({'city': city_name, 'date': date_list[-1], 'time': times_list[-4:], 'status': conditions_list[-4:],
        ...
        for html in htmls:
            df = asyncio.run(_extractor(html))
            data = data.append([df])

        data.to_csv(f'weather forecast.csv', index=False)
